...
- **Python 3.13** — LEOHS must be run in its own environment
...
### 1. Create a clean Python 3.13 environment (recommended name: `leohs_env`)
```bash
conda create -n leohs_env python=3.13 -y
...
If the user sets this value to True, the filter will only keep AOT values <0.3, allowing average and clear pixels. 
...
## ⬆️ Latest Update for 1.2
- LEOHS runs in Python 3.13 instead of 3.10.
- Updated East/West point sampling strategy to minimize BRDF. 
