...
def generate_equalA_points (AOI,points_n):
    ...
    rows, cols=np.where(mask)#Sample points from zthe mask raster grid
...
def create_overlap_points_gdf(points_gdf, WRS_gdf):
    points_gdf=points_gdf.to_crs(WRS_gdf.crs)
    joined_gdf=gpd.overlay(points_gdf, WRS_gdf[['geometry', 'LS7_row', 'LS7_path', 'LS8_path']], how='intersection')
    joined_gdf=joined_gdf[['LS7_row', 'LS7_path', 'LS8_path', 'geometry']]
    joined_gdf=joined_gdf.sample(frac=1).reset_index(drop=True)
    joined_gdf=joined_gdf.drop_duplicates(subset='geometry')
    return joined_gdf.reset_index(drop=True)
